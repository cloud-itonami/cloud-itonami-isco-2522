(ns sysadmin.actor
  "SystemsAdministrationActor — the ISCO-08 2522 community sysadmin actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one sysadmin operation request
  ...
  The unconditional invariant: the SystemsAdministrationAdvisor can never
  directly commit a record the SystemsAdministrationGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [sysadmin.advisor :as advisor]
            [sysadmin.governor :as governor]
            [sysadmin.store :as store]))

(defn build-graph
  "Build a compiled SystemsAdministrationActor graph. `store` implements
  `sysadmin.store/Store`. `advisor` implements
  `sysadmin.advisor/Advisor` (defaults to `mock-advisor`).
